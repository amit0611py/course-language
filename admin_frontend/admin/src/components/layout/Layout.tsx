...
const NAV = [
  ...
  { to: "/paid",      icon: "🔒", label: "Paid Content" },
];
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: A.bg, color: A.text, fontFamily: "'Inter', sans-serif" }}>
      <aside style={{
        ...
        background: A.sidebar, borderRight: `1px solid ${A.sidebarBorder}`,
        ...
        <div style={{
          ...
          {!collapsed && <span style={{ fontSize: 13, fontWeight: 700, color: A.text, whiteSpace: "nowrap" }}>Content Admin</span>}
          <button onClick={() => setCollapsed(c => !c)} style={{ marginLeft: "auto", background: "transparent", color: A.muted, border: "none", cursor: "pointer", fontSize: 14, flexShrink: 0, padding: 2 }}>
            {collapsed ? "→" : "←"}
          </button>
        </div>
          ...
          {NAV.map(item => {
            ...
            return (
              <NavLink key={item.to} to={item.to} style={{
                ...
                padding: "8px 10px", borderRadius: 8, marginBottom: 2, textDecoration: "none",
        ...
        {!collapsed && (
          <div style={{ padding: "12px 16px", borderTop: `1px solid ${A.sidebarBorder}`, fontSize: 10, color: A.dim }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <header style={{
          ...
          justifyContent: "space-between", flexShrink: 0, background: A.surface,
        }}>
          ...
          <a href="http://localhost:5173" target="_blank" rel="noreferrer" style={{
            ...
            background: A.card, border: `1px solid ${A.border}`, borderRadius: 7, padding: "4px 12px",
          }}>
